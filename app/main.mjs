...
// All imperative app behavior. Runs once on mount; returns a cleanup
// function, so it plugs straight into useEffect(initApp, []).
export function initApp() {
    ...
    return () => {
      ...
      if (recordPlaybackTimer) clearTimeout(recordPlaybackTimer);
      if (timelinePlayTimer) clearTimeout(timelinePlayTimer);
      clearTimeout(dragEndTimer);
      if (lastVideoUrl) URL.revokeObjectURL(lastVideoUrl);
